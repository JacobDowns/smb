...
class RandomInputs(CommonInputs1):
    ...
    def randomize(self):
        self.ts_rand = self.ts + np.sqrt(100.) * np.random.randn(len(self.ts))
        self.Ls_rand = self.Ls + np.sqrt(1000.0) * np.random.randn(len(self.Ls))
    ...
    ### Return L
    def get_L(self, t):
        return self.L_interp(t)


    ### Return dLdt
    def get_dLdt(self, t):
        return self.dLdt_interp(t)
